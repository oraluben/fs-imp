...
type Formula =
    ...
    member this.DisjunctionNormalForm : Formula =
        ...
        | _ -> this
    member this.sharedVars : Set<string> =
        match this with
        | Bool(IsTrue(Compare(Name(n), _, _))) | Bool(IsFalse(Compare(Name(n), _, _))) -> Set.singleton n
        | Disjunction(f1, f2) | Conjunction(f1, f2) -> Set.union f1.sharedVars f2.sharedVars
        | _ -> Set.empty
